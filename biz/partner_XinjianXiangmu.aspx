...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (Request["commit"] == "好了，提交")//Request.HttpMethod=="POST"
        {
            ...
            {
                Response.Redirect("partner_index.aspx", true);
            }
    ...
    private void selectContent()
    {
        ...
        begintime.Value = DateTime.Now.AddDays(2).ToString("yyyy-MM-dd 00:00:00");
        //endtime.Value = DateTime.Now.AddMonths(2).ToString("yyyy-MM-dd 00:00:00");
    ...
    private void addContent()
    {
        ...
        team.User_id = Helper.GetInt(CookieUtils.GetCookieValue("partner", FileUtils.GetKey()), 0);
        //if (projectType.Value != "")
        //{
        team.Team_type = "normal";//默认团购项目
        //}
        ...
        DateTime etime = btime.AddMonths(2);// AS.Common.Utils.Helper.GetDateTime(endtime.Value, DateTime.MinValue);
        ...
        //else
        //{
        team.isPredict = 1;// AS.Common.Utils.Helper.GetInt(Request["isPredict"], 1);
        //}
        //if (rech_time.Value != "")
        //{
        //    try
        ...
        //}
        team.Reach_time = team.Begin_time;
        ...
        team.Expire_time = team.Begin_time.AddMonths(3);

        team.System = "N";
        team.End_time = team.Begin_time.AddMonths(2);// Convert.ToDateTime(endtime.Value.ToString());
        ...
        if (team.Conduser == "Y")
        {
            ...
            if (System.DateTime.Now > team.End_time /* Convert.ToDateTime(endtime.Value) */ )
            {
            ...
            if (team.Delivery == "coupon" && AS.Common.Utils.Utility.Getbulletin(team.bulletin) != "")
            {
                ...
                if (a > 0)
                {
                    if (AS.Common.Utils.Helper.GetInt(team.open_invent, 0) == 1)//开启库存
                    {
                        if (team.productid == 0)
                        {
                            //SetSuccess("<script language='javascript'>if(!confirm('友情提示：已开启库存功能，是否进行出入库操作！')){location.href='partner_index.aspx'}else{X.get('" + PageValue.WebRoot + "ajax_coupon.aspx?action=invent&p=d&inventid=" + a + "')}<" + "/script>");
                        }
                        else
                        {
            ...
            else
            {
                ...
                if (a > 0)
                {
                    if (AS.Common.Utils.Helper.GetInt(team.open_invent, 0) == 1)//开启库存
                    {
                        if (team.productid == 0)
                        {
                            //SetSuccess("<script language='javascript'>if(!confirm('友情提示：已开启库存功能，是否进行出入库操作！')){location.href='partner_index.aspx'}else{X.get('ajax_coupon.aspx?action=invent&p=d&inventid=" + a + "')}<" + "/script>");
                        }
                        else
                        {
                            SetSuccess("友情提示：添加项目成功");
...
<script language='javascript'>
    if (!confirm('友情提示：已开启库存功能，是否进行出入库操作！')) { location.href = 'partner_index.aspx' } else { X.get('/manage/ajax_coupon.aspx?action=invent&p=d&inventid=<%=team.Id %>') }</script>
<%}%>
                                ...
                                <div class="sect">
                                    ...
                                    <div class="field" id="field_limit">
                                        ...
                                        <asp:DropDownList ID="Buyonce" runat="server" class="f-input" Style="width: 160px;">
                                            <asp:ListItem Value="Y">仅购买一次</asp:ListItem>
                                            <asp:ListItem Value="N" Selected="True">可购买多次</asp:ListItem>
                                        </asp:DropDownList>
                                    </div>
                                    <div class="field">
                                        <label>
                                    ...
                                    <div class="field">
                                        <label>
                                            手机商品图片</label>
                                        <asp:FileUpload ID="PhoneImg" name="PhoneImg" runat="server" class="f-input" Style="float: left;
                                            width: 50%;" />&nbsp;
                                    </div>
