        ...
        if ( Request.HttpMethod=="POST") 
        {
            ...
            else
            {
                ...
                if (count > 0)
                {
                    ...
                    {
                        SetSuccess(user.Username + "减去金额成功！减去"+ASSystem.currency + (inputmoeny * -1) + ",减去积分成功！减去"+ASSystem.currency + (userscore * -1));
                    }
                    else if (inputmoeny < 0 && userscore == 0)
                    {
                    ...
                    {
                        SetSuccess(user.Username + "充值积分成功！充值" + ASSystem.currency + userscore);
                    }
                    else if (userscore < 0 && inputmoeny == 0)
                    {
                        SetSuccess(user.Username + "减去积分成功！减去" + ASSystem.currency + (userscore * -1));
                    }
                    else if (inputmoeny < 0 && userscore > 0)
                    {
                        SetSuccess(user.Username + "减去金额成功！减去" + ASSystem.currency + (inputmoeny * -1) + ",充值积分成功！充值"+ASSystem.currency + userscore);
                    }
                    else if (inputmoeny > 0 && userscore < 0)
                    {
                        SetSuccess(user.Username + "充值金额成功！充值" + ASSystem.currency + inputmoeny + ",减去积分成功！减去"+ASSystem.currency + (userscore * -1));
                    }
                    else if (inputmoeny > 0 && userscore > 0)
                    {
                        SetSuccess(user.Username + "充值金额成功! 充值" + ASSystem.currency + inputmoeny + ",充值积分成功! 充值"+ASSystem.currency + userscore);
                    }
                }
                else 
                {
...
<div id="order-pay-dialog" class="order-pay-dialog-c" style="width: 330px;">
    ...
    <table width="96%" align="center" class="coupons-table-xq">
        ...
        <tr>
            <td><input type="submit" ask='确定给该用户充值吗?' name="but"  value="充值" group="user" class="formbutton validator" onclick="return checkvale();" /></td>
        </tr>
