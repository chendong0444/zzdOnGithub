...
<%@ Import Namespace="AS.AdminEvent" %>
...
<script runat="server">
    ...
    {
        base.OnLoad(e);
        del = Helper.GetInt(Request["delete"], 0);
        ...
        if (id > 0)
        {
            ...
            string message;
            string error = OrderEvent.Manager_Cash(id, AsUser.Id);
            //Maticsoft.BLL.Order.Manager_Cash(id, AsUser.Id);
            if (error == String.Empty)
            {
                //Maticsoft.BLL.Order.draw_action(id);//执行抽奖动作
                //Maticsoft.BLL.inventorylog.Invent_action(id,AsUser.Id);//点击‘现金’动作时候，进行出入库的动作


                //订单付款成功提醒
                DataRowObject orderrow = null;
                List<Hashtable> hashtable = null;
                System.Data.DataTable table = new System.Data.DataTable();
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    hashtable = session.GetData.GetDataList("select id,Origin,Credit,Money from [order] where id=" + id);
                }
                    //orderrow = db.SqlExecDataRow("select id,Origin,Credit,Money from [order] where id=" + id);
                if (hashtable != null)
                    SetSuccess("掉单订单ID" + id + "更改为已付款成功,其中余额付款" + hashtable[0]["credit"]  + "元,在线支付" + hashtable[0]["money"] + "元");
                Response.Redirect("Dingdan_DangqiDingdan.aspx");
            }
            else
                SetError(error);
        ...
        if (del > 0)
        {
            ...
            AS.AdminEvent.OrderEvent.Del_Order(adminid, del, Request.UrlReferrer.ToString());
        }
    ...
    private void InitData()
    {
        ...
        filter.StateIn = "'pay','unpay','cancel','refunding','refund'";
        filter.PageSize = 30;
        ...
        foreach (IOrder order in list_order)
        {
            ...
            {
                sb2.Append("<td class='op'><a class='deal-title' href='Dingdan_DangqiDingdan.aspx?id=" + order.Id + "' ask='确定本单为现金付款?'>现金</a>｜<a class='deal-title' href='Dingdan_DangqiDingdan.aspx?delete=" + order.Id + "' ask='确定删除本单吗？'>删除</a>｜<a class='ajaxlink' href='ajax_manage.aspx?action=orderview&orderview=" + order.Id + "'>详情</a></td>");
            }
                    ...
                    <div id="content" class="coupons-box clear mainwide">
                        <div class="box clear">
                            <div class="box-content">
                                <div class="head">
                                    <h2>
                                        所有订单</h2><form runat="server" method="get">
                                <div class="search" style="margin-right:0px;">
                                   ...
                                   订单状态：<select name="state" class="h-input">
                                        ...
                                        &nbsp;&nbsp; 内容：<input class="h-input" type="text" style="width: 120px;" name="type_name"
