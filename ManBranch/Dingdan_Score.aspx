...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["state"]))
        {
            ...
            if (Request["state"] == "1")
            {
                filter.State = "scoreunpay";
            }
            else if (Request["state"] == "2")
            {
                filter.State = "scorepay";
            }
        }
        ...
        {
            url = url + "&fromfinishtime=" + Request.QueryString["fromfinishtime"];
    ...
    private void InitData()
    {
        ...
        sb1.Append("<th width='20%'>项目</th>");
        sb1.Append("<th width='20%'>用户</th>");
        sb1.Append("<th width='10%'>数量</th>");
        sb1.Append("<th width='10%'>支付状态</th>");
        sb1.Append("<th width='10%'>消耗积分</th>");
        sb1.Append("<th width='20%'>操作</th>");
        sb1.Append("</tr>");
        url = url + "&page={0}";
        url = "Dingdan_Score.aspx?" + url.Substring(1);
        filter.StateIn = "'scorepay','scoreunpay'";
        filter.PageSize = 30;
        ...
        foreach (IOrder order in list_order)
        {
            ...
            sb2.Append("<td>" + order.totalscore + "</td>");
            if (order.State == "scorepay")
            {
                sb2.Append("<td class='op'><a class='ajaxlink' href='ajax_manage.aspx?action=orderview&orderview=" + order.Id + "'>详情</a></td>");
            }
            else
            {
                sb2.Append("<td class='op'><a class='deal-title'href='Dingdan_Score.aspx?delete=" + order.Id + "' ask='确定删除本单吗？'>删除</a>｜<a class='ajaxlink' href='ajax_manage.aspx?action=orderview&orderview=" + order.Id + "'>详情</a></td>");
            }
            sb2.Append("</tr>");
        }
                    ...
                    <div id="content" class="coupons-box clear mainwide">
                        <div class="box clear">
                            <div class="box-content">
                                <div class="head">
                                    <h2>
                                        积分订单</h2><form id="Form1" runat="server" method="get">
                                <div class="search">
                                    订单状态：<select name="state" class="h-input">
                                        ...
                                        <option value="2" <%if(Request.QueryString["state"] == "2"){ %>selected="selected"
                                            <%} %>>付款</option>
                                    </select>&nbsp;&nbsp; 生成订单时间：<input type="text" class="h-input" datatype="date" name="begintime"
                                                ...
                                                <%} %> />--<input type="text" class="h-input" datatype="date" name="endfinishtime"
                                                    ...
                                                    <%} %> />&nbsp;&nbsp; 筛选条件：<select name="select_type" class="h-input">
                                                        ...
                                                        <option value="256" <%if(Request.QueryString["select_type"] == "256"){ %>selected="selected"
                                                            <%} %>>派送地址</option>
                                                    </select>&nbsp;&nbsp; 内容：<input type="text" class="h-input" style="width: 120px;"
