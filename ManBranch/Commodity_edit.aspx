...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (_system["userreview_rebate"] != null)
        {
            comment = _system["userreview_rebate"].ToString();
            commentscore.Value = comment;
        }
        //第一次提及,赋初值
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["id"]))
            {
                ...
                if (mpartner != null)
                {
                    ...
                    shanghu.SelectedItem.Text = mpartner.Title;
                }
                //市场价
                ...
                else
                {
                    ...
                    //弹出层
                    type1.Attributes.Add("style", "display:block");
                    type0.Attributes.Add("style", "display:none");
                    if (team.freighttype != 0)
                    {
    ...
    protected void initdorp()
    {
        ...
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
            productlist = session.Product.GetList(productft);
        }
        DataTable dtProduct = new DataTable();
        if (productlist.Count>0)
        {
            dtProduct=AS.Common.Utils.Helper.ToDataTable(productlist.ToList());
        }
        if (dtProduct != null)
        {
            if (dtProduct.Rows.Count > 0)
            {
                ...
                {
                    ddlProduct.Items.Add(new ListItem(dtProduct.Rows[i]["id"].ToString() + ":" + dtProduct.Rows[i]["Productname"].ToString(), dtProduct.Rows[i]["id"].ToString()));
                    if (team.productid == 0)
                    {
                        continue;
                    }

                    if (product.productname == dtProduct.Rows[i]["Productname"].ToString())
                    {
                        ddlProduct.SelectedIndex = i + 1;
                    }
                }
            }
        }
    ...
    private void selectContent()
    {
        ...
        if (system!=null)
        {
            createbonus.Value = GetMoney(system.invitecredit);
        }       
        ...
        partnerft.AddSortOrder(PartnerFilter.ID_DESC);
        DataTable dt = null;
        IList<IPartner>  listp = null;
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
            listp = session.Partners.GetList(partnerft);
        }
        if (listp.Count>0)
        {
            dt = AS.Common.Utils.Helper.ToDataTable(listp.ToList());
        }
    ...
    private void addContent()
    {
        ...
        team.commentscore = Convert.ToDecimal( Helper.GetDouble(commentscore.Value, 0));//评论返利的金额
        ...
        {
            SetError("图片不能为空！");
            Response.Redirect("Commodity_edit.aspx?id=" + team.Id);
            Response.End();
        }
                    ...
                    <div id="content" class="box-content clear mainwide">
                                ...
                                <div class="sect">
                                    ...
                                    <div class="field">
                                        <label>
                                            商品名称</label>
                                        <input type="text" size="30" name="product" id="createproduct" class="f-input" group="g" style=" width:692px"
                                    ...
                                    <div id="img0" class="field">
                                        <label>
                                            引用其他图片</label>
                                        <input type="text" size="30" name="imget" id="imget" class="f-input"   style=" width:692px" runat="server" />
                                    </div>
                                    ...
                                    <div class="field">
                                        ...
                                        <div style="margin-top: 5px;">
                                            <div id="express" class="daoru">
                                                <input id="zonediv2" type="radio" name="radio" value="express" checked="true" runat="server" />快递</div>
                                        </div>
                                    </div>
                                    <div id="express_zone_express">
                                        ...
                                        <div class="field" id="type1" style="display: none" runat="server">
                                            <label>
                                                快递费模板</label>
                                        ...
                                        <div class="field" id="type0" runat="server">
                                            <label>
                                                快递费用</label>
