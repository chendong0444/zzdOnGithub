...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        form.Action = GetUrl("不同价格订单", "shopcart_notpriceation.aspx?orderid=" + orderid);
        ...
        if (Request.Form["btnadd"] == "确认订单，付款")
        {
            ...
            string allmoney = Request["totalprice"];
            using (IDataSession session = Store.OpenSession(false))
            {
            ...
            if (ordermodel != null)
            {
                if (ordermodel.OrderDetail.Count > 0)
                {
                    ...
                    IOrderDetail orderdemodel = ordermodel.OrderDetail[0];
                    if (orderdemodel != null)
                    {
                        if (Helper.GetInt(Request["num"], 0) > 0)
                        {
                            orderdemodel.Num = Helper.GetInt(Request["num"], 0);
                            string str =  orderdemodel.result.Substring(0, orderdemodel.result.LastIndexOf(',')) + ",数量:[" + orderdemodel.Num+"]}";
                            orderdemodel.result = str;
                            using (IDataSession session = Store.OpenSession(false))
                            {
...
<form id="form" runat="server">
    <script type="text/javascript">
        ...
        document.getElementById("totalprice").innerHTML = t.toFixed(2);
        var fare = "0";
        if (document.getElementById("deal-express-price") != null) {
            var fare = document.getElementById("deal-express-price").innerHTML;
            getfare();
        }
            document.getElementById("totalprice").innerHTML = (t + parseFloat(fare)).toFixed(2);
        }
        function show2(teamid, num, price, max, min, per, inventory, open_invent) {
        ...
        var s = parseFloat(document.getElementById("totalprice").innerHTML);
                            ...
                            <table class="order-table">
                                ...
                                <tr>
                                    ...
                                    <td t="totalnum" tid="<%=model.Teamid %>" class="deal-buy-quantity">
                                          <div class="num_div">
                                            <span class="crease decrease" onclick="show2('<%=teammodel.Id %>','num','price','<%=teammodel.Per_number %>','<%=teammodel.Per_minnumber %>','<%=teammodel.Per_number %>','<%=AS.Common.Utils.Utility.GetInventNum(ordermodel.OrderDetail[0].result,teammodel.invent_result) %>','<%=teammodel.open_invent %>')"></span><span class="crease" onclick="show1('<%=teammodel.Id %>','num','price','total','<%=teammodel.Max_number %>','<%=teammodel.Per_minnumber %>','<%=teammodel.Per_number %>','<%=AS.Common.Utils.Utility.GetInventNum(ordermodel.OrderDetail[0].result,teammodel.invent_result)%>','<%=teammodel.open_invent %>')"></span>
                                            <input type="text" id="num" idval="<%=teammodel.Id %>" onBlur="show('<%=teammodel.Id %>','num','price','total','<%=teammodel.Max_number %>','<%=teammodel.Per_minnumber %>','<%=teammodel.Per_number %>','<%=AS.Common.Utils.Utility.GetInventNum(ordermodel.OrderDetail[0].result,teammodel.invent_result)%>','<%=teammodel.open_invent %>')"
                                                class="input-text f-input1 deal-buy-quantity-input" maxlength="4" name="num"
                                                value="<%=ordermodel.OrderDetail[0].Num %>" category="product" />
                                            <br />
                                            <br />
                                    ...
                                    <td class="deal-buy-price" id="deal-buy-price">
                                        <span class="money"><span>
                                            <%=ASSystem.currency%><span id="price"><%=model.Teamprice%></span>
                                    </td>
                                ...
                                <tr class="order-total">
                                    ...
                                    <td class="deal-buy-total">
                                        <%=ASSystem.currency%><span total="totalprice" id="totalprice"><%=totalprice%></span>
                                    </td>
                                    <td></td>
                                </tr>
                            </table>
                ...
                <div class="sbox">
                    <div class="sbox-content">
                        ...
                        <div class="credit">
                            <h2>账户余额</h2>
                            <p>
                                您的账户余额：<span class="money"><%=ASSystem.currency %></span><%=user.Money %>
                            </p>
                        </div>
                        <div class="r-bottom">
                        </div>
