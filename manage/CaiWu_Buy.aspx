...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        base.OnLoad(e);
        // 判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Flow_Buy))
        {
            SetError("你不具有查看在线支付的权限！");
        ...
        if (Request.QueryString["item"] != null)
        {
            ...
            foreach (string ids in item)
            {
                ...
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    ...
                    {
                        SetSuccess("删除选中成功！");
                        AS.Common.Utils.WebUtils.LogWrite("删除财务记录日志", "《删除在线支付记录》FlowId:" + id + ",操作员ID：" + AS.Common.Utils.WebUtils.GetLoginAdminID() + "");
                    }
                    else
                    {
                        SetError("删除选中失败！");
            ...
            AS.AdminEvent.ConfigEvent.AddOprationLog(AS.Common.Utils.Helper.GetInt(AS.Common.Utils.CookieUtils.GetCookieValue("admin", key), 0), "删除在线支付记录", "删除在线支付记录 ID:" + Request.QueryString["item"].ToString(), DateTime.Now);
            Response.Redirect("CaiWu_Buy.aspx?page=" + Helper.GetInt(Request.QueryString["url"], 1));
        }
    ...
    private void InitData()
    {
        ...
        sb1.Append("<th width='20%'>动作</th>");
        sb1.Append("<th width='20%'>金额</th>");
        sb1.Append("<th width='30%'>时间</th>");
        sb1.Append("</tr>");

        url = url + "&page={0}";
        url = "CaiWu_Buy.aspx?" + url.Substring(1);
        filter.Action = "buy";
        filter.PageSize = 30;
        ...
        if (iListFlow != null && iListFlow.Count > 0)
        {
                    ...
                    <div id="content" class="box-content clear mainwide">
                        <div class="box clear">
                            <div class="box-content">
                                <div class="head">
                                    <h2>
                                        在线支付</h2><form id="form1" runat="server" method="get">
                                <div class="search">
                                时&nbsp;间：
 ...
 <script type="text/javascript">
     ...
     function GetDeleteItem(url) {
         ...
         if (str.length > 0) {
             ...
             if (istrue) {
                 window.location = "CaiWu_Buy.aspx?item=" + $("#items").val() + "&url=" + urls ;
             }
