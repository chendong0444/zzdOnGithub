...
<script runat="server">
    ...
    public string hideyhq = String.Empty;
    ...
    {
        base.OnLoad(e);
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Team_Ediit))
        {
            SetError("你不具有编辑项目的权限！");
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["del"] != null && Request.QueryString["image"] != null)
            {
                ...
                {
                    team = session.Teams.GetByID(Helper.GetInt(Request.QueryString["del"].ToString(),0));
                }
                int index = Helper.GetInt(Request.QueryString["image"].ToString(),0);
                if (team != null)
                {
                    teams.Id = team.Id;
                    if (index == 1)
    ...
    private void getContent()
    {
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
            team = session.Teams.GetByID(Helper.GetInt(Request.QueryString["id"],0));
        }
        if (team != null)
        {
        ...
        IProduct prodmodel = GetProductData(team.productid.ToString(), "");
        if (prodmodel != null)
        {
            ddlProduct.Value = prodmodel.productname;
            prohidden.Value = prodmodel.id.ToString();
        }
        else
        {
            ddlProduct.Value = "-----";
        ...
        //this.ddlbrand.SelectedIndex = team.brand_id;
        host = team.teamhost;
        ...
        #region 项目的颜色或者尺寸
        if (team.bulletin != null && team.invent_result != null) //项目有规格则隐藏优惠券等
        {
            if (team.bulletin.Replace("{","").Replace("}","") !="")
            {
            ...
            if (bulletinteam[i] != "")
            {
                if (bulletinteam[i].Split(':')[0] != "" && bulletinteam[i].Split(':')[1] != "")
                {
                    ...
                    if (invenStatus == 0)
                    {
                        ...
                        bulletin += "<input type=\"button\"  class=\"formbutton\"  value=\"删除\" onclick='deleteitem(this," + '"' + "tb" + '"' + ");'><br />";
                    }
                    else
                    {
                        bulletin += "属性：<input class=\"h-input\" type=\"text\" name=\"StuNamea" + i + "\" value=" + bulletinteam[i].Split(':')[0] + " readonly>数值：<input style=\"width:500px\" class=\"h-input\" type=\"text\" name=\"Stuvaluea" + i + "\" value=" + bulletinteam[i].Split(':')[1].Replace("[", "").Replace("]", "") + " readonly><br />";
                    }
                    bulletin += "</td>";
                    bulletin += "</tr>";
                }
        ...
        #endregion
        comment = team.commentscore.ToString();//评论的返利金额  // teamnew = team.teamnew;
        ...
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
            cit = session.Citys.GetByID(team.City_id);
        }
        if (cit == null)
        {
            team.City_id = 0;
        ...
        //添加api分类
        CategoryFilter apicate = new CategoryFilter();
        apicate.Zone = "group";
        DataTable dt = new DataTable();
        IList<ICategory> listcate = null;
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
            listcate = session.Category.GetList(apicate);
        }
        if (listcate.Count > 0)
        {
            dt = AS.Common.Utils.Helper.ToDataTable(listcate.ToList());
        }
        BindApiData(dt, 0, "");
        ...
        if (team != null)
        {
            ...
            if (team.Team_type == "seconds" || team.Team_type == "normal")
            {
            ...
            expiretime.Value = team.Expire_time.ToString("yyyy-MM-dd HH:mm:ss");
            createsummary.Value = team.Summary;
            ...
            if (team.PhoneImg!=null)
            {
                PImageSet.InnerText = team.PhoneImg;
            }
            IPartner partmodel = GetPartnerData(team.Partner_id.ToString(), "");
            if (partmodel != null)
            {
                shanghu.Value = partmodel.Id.ToString() + ":" + partmodel.Title;
                shHidden.Value = partmodel.Id.ToString();// 记录商户id查找分站 
            }
            else
            {
                shanghu.Value = "";
            ...
            if (!String.IsNullOrEmpty(team.Image1))
            {
            ...
            if (!String.IsNullOrEmpty(team.Image2))
            {
    ...
    private void updateContent()
    {
        ...
        if (Helper.GetInt(city.SelectedItem.Value,0) > -1)
        {

            team.City_id = Helper.GetInt(city.SelectedItem.Value,0);
        }
        ICity cit = null;
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
        ...
        if (shanghu.Value != "" && shanghu.Value != null)
        {
            ...
            {
                if (Helper.GetInt(Request.Form["fenzhan"].ToString(),0) == 0)
                {
                    team.branch_id = 0;
                }
                else
                {
                    team.branch_id = Helper.GetInt(Request.Form["fenzhan"].ToString(),0);
                }
            ...
            {
                if (Helper.GetInt(Request.Form["xiaoshou"].ToString(),0) == 0)
                {
                    team.sale_id = 0;
                }
                else
                {
                    team.sale_id = Helper.GetInt(Request.Form["xiaoshou"].ToString(),0);
                }
            }
        }
        ...
        {
            team.Group_id = Helper.GetInt(Request.Form["groupType"].ToString(),0);
        }
        //项目分类的关键字
        ...
        team.Min_number = Helper.GetInt(minnumber.Value,0);
        team.Max_number = Helper.GetInt(maxnumber.Value,0);
        team.Per_number = Helper.GetInt(pernumber.Value,0);
        team.Per_minnumber = Convert.ToInt32(per_minnumber.Value);
        team.score = Helper.GetInt(score.Value,0);
        ...
        {
            team.areaid = Helper.GetInt(Request.Form["ddlarea"],0);
        }
        if (Request.Form["ddlarea"] != null && Request.Form["ddlcircle"] != null)
        {
            team.circleid = Helper.GetInt(Request.Form["ddlcircle"],0);
        }
        else
        {
            team.circleid = 0;
        ...
        if (shanghu.Value != "" && shanghu.Value != null)
        {
            IPartner ipartner = null;
            string strpart = shanghu.Value;
            int strindexof = strpart.IndexOf(":");
            if (strindexof != -1)
            {
                ipartner = GetPartnerData(strpart.Substring(0, strindexof), "");
            }
            if (ipartner != null)
            {
        ...
        team.Bonus = Helper.GetInt(createbonus.Value,0);
        ...
        if (PhoneImg.FileName != null && PhoneImg.FileName.ToString() != "")
        {
            #region 上传图片1
            ...
            createUpload = createUpload.Replace("~", "");
            team.PhoneImg = createUpload + pictureName;
            #endregion
        }
        else
        {
        ...
        team.teamhost = Helper.GetInt(Request["hostt"], 0);
    ...
    {
        int teamid = Helper.GetInt(Request.QueryString["id"],0);
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false)) 
        {
        ...
        string productid = "0";
        string strproduct = ddlProduct.Value;
        int strinof = strproduct.IndexOf(":");
        if (strinof != -1)
        {
            iproduct = GetProductData(ddlProduct.Value.Substring(0, strinof), "");
        }
        if (iproduct != null)
        {
        ...
        else
        {
            ...
            Response.Redirect(WebRoot + "manage/Project_BianjiXiangmu.aspx?id=" + team.Id);
            Response.End();
            return;
        }
        ...
        if (productid != "0")
        {
            ...
            {
               //查看当前项目是否是优惠券项目，
                if (!string.IsNullOrEmpty(productmodel.invent_result))
                {
                    if (team.Delivery == "coupon")
                    {
                        SetError("优惠券项目没有规格,请选择其他产品");
                        Response.Redirect(WebRoot + "manage/Project_BianjiXiangmu.aspx?id=" + team.Id);
                        Response.End();
                        return;
                    }
                ...
                for (int i = 0; i < bulletinteam.Length; i++)
                {
                    ...
                    if (bulletinteam[i] != "")
                    {
                        ...
                        if (bulletinteam[i].Split(':')[0] != "" && bulletinteam[i].Split(':')[1] != "")
                        {
                            bulletin += "<tr>";
                            bulletin += "<td>";
                            bulletin += "属性：<input class=\"h-input\"  type=\"text\" name=\"StuNamea" + i + "\" value=" + bulletinteam[i].Split(':')[0] + ">数值：<input style=\"width:500px\" class=\"h-input\"  type=\"text\" name=\"Stuvaluea" + i + "\" value=" + bulletinteam[i].Split(':')[1].Replace("[", "").Replace("]", "") + "><input  class=\"formbutton\" type=\"button\" value=\"删除\" onclick='deleteitem(this," + '"' + "tb" + '"' + ");'><br />";
                            bulletin += "</td>";
                            bulletin += "</tr>";
                ...
                {
                    shanghu.Value = partmodel.Id.ToString() + ":" + partmodel.Title;
                    shHidden.Value = partmodel.Id.ToString();
                }
            }
        }
        else
...
<script language='javascript'>
    if (!confirm('友情提示：该项目已开启库存功能，是否进行出入库操作！')) { location.href = 'Project_DangqianXiangmu.aspx' } else { X.get('/manage/ajax_coupon.aspx?action=invent&p=d&inventid=<%=team.Id %>') }</script>
<%}%>
<script type="text/javascript" language="javascript">
    ...
    function additem(id) {
        ...
        if (row != null) {
            ...
            cell.innerHTML = "属性：<input class=\"h-input\" type=\"text\" name=\"StuName" + num + "\">数值：<input style=\"width:500px\" class=\"h-input\" type=\"text\" name=\"Stuvalue" + num + "\"><input class=\"formbutton\"  type=\"button\" value=\"删除\" onclick='deleteitem(this," + '"' + "tb" + '"' + ");'><br />";
        }
        num++;
    ...
    $(document).ready(function () {
        jQuery("#city").change(function () {
            ...
            if (str != "") {
                ...
                $.ajax({
                    type: "POST",
                    url: "manage_ajax_othercitys.aspx?type=edit",
...
<script type="text/javascript">
    ...
    $(function () {
        var branchid=<%=team.branch_id%>;
        var saleid= jQuery("#shanghu").val().substring(0, jQuery("#shanghu").val().indexOf(":"));
        if (branchid>0) {
            $("#branchzeng").show();
            $.ajax({
                type: "POST",
                url: "manage_ajax_getbranch.aspx",
                data: { "partnerid": branchid ,"team_id":<%=team.Id%>},
                success: function (msg) {
                    $("#all_branch").html(msg);
                }
            });
        ...
        if (saleid>0) {
            ...
            $.ajax({
                ...
                data: { "partnerid": saleid,"team_id":<%=team.Id%>},
                success: function (msg) {
                    $("#all_Sales").html(msg);
    ...
    function GetSale() {
        var str = jQuery("#shanghu").val().substring(0, jQuery("#shanghu").val().indexOf(":"));
        if (str != "" & str != "0") {
            $("#saleceng").show();
            $.ajax({
                type: "POST",
                    ...
                    <div id="content" class="box-content">
                        ...
                        <div class="sect">
                            ...
                            <div class="field">
                                <label>
                                    产品</label>
                                <input type="text" id="ddlProduct" name="ddlProduct" class="f-input" Style="width: 360px;" runat="server" 
                            ...
                            <div class="field" id="divIsPredict" runat="server">
                                <label>
                                    团购预告</label>
                                <select id="selIsPredict" class="f-input" name="isPredict" runat="server" style="float: left;
                                    width: 160px;">
                            ...
                            <div class="field" style='<%=hidepoint %>'>
                                <label>
                                    关键字</label>
                                <input id="birds" class="f-input" size="30" name="birds" style="width: 160px;" type="text"
                                    value='<%=keyword %>' />
                            ...
                            <div class="field" id="dhost" style='<%=hidepoint %>'>
                                ...
                                <select id="hostt" name="hostt" style="width: 160px;" class="f-input">
                                    ...
                                    <option value="1">新品</option>
                                    <option value="2">推荐</option>
                                    <option value="3" selected="selected">自然新品</option>
                                    <% }
                                       %>
                                </select>
                            ...
                            <div class="field">
                                ...
                                <%if (invenStatus == 0)
                                  { %>
                                <input type="button" class="formbutton" name="btnAddFile" value="添加" onclick="additem('tb')" />
                                <%} %>
                                <font style='color: red'>例如：属性填写：颜色 数值填写：红色|黄色|蓝色</font>
                            </div>
                            <div class="field">
                                <label>
                                </label>
                            ...
                            <div class="field">
                                ...
                                <input type="text" size="6" name="team_create_now_update" id="team_create_now_update"
                                    class="h-input number" value="<%=update_value %>" require="true" group="g" maxlength="4"
                            ...
                            <div class="field">
                                <label>
                                    开始时间</label>
                                <input type="text" name="begin_time" id="begintime" group="g" require="true" datatype="datetime"
                                    onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" class="date" runat="server" />
                                <label>
                                    结束时间</label><input type="text" name="end_time" id="endtime" group="g" require="true"
                                        onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" datatype="datetime" class="date"
                                        runat="server" />
                                <div id="ctime" style='<%=hidepoint %>'>
                                    <label>
                                        优惠券有效期</label>
                                    <input type="text" name="expire_time" id="expiretime" class="number" 
                                        group="g" datatype="datetime" runat="server" />
                            ...
                            <div class="field">
                                <label>
                                    成团时间</label>
                                <input type="text" name="rech_time" id="rech_time" datatype="datetime" class="date"
                                    onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" value="" runat="server" />
                            ...
                            <div class="field" id="summery" style='<%=hidepoint%>'>
                                ...
                                <div style="float: left;">
                                    <textarea cols="45" rows="5" name="summary" id="createsummary" class="f-textarea xheditor {upImgUrl:'../upload.aspx?immediate=1&t=0',urlType:'abs'}"
                            ...
                            <div class="field">
                                ...
                                <input type="text" size="10" name="invent_war" id="invent_war" class="number" value="0"
                                    group="g" datatype="number" runat="server" /><span class="inputtip">库存报警数量,请填写数字，否则为0</span>
                            </div>
                            <!--xxk-->
                                ...
                                <label>
                                    商户</label><div>
                                        <input type="text" id="shanghu" runat="server"  style="float: left;width:30%;" class="f-input"
                                            onclick="this.focus();this.select();"  onblur="GetSale();"  />
                            ...
                            <div class="field">
                                <label>
                                    商品名称</label>
                                <input type="text" style="width: 49%" name="product" id="createproduct" class="f-input"
                                    group="g" datatype="require" require="true" runat="server" />
                            </div>
                            <div class="field">
                                <label>
                                   手机商品图片</label>
                                <asp:FileUpload ID="PhoneImg" Style="width: 50%" runat="server" size="30" group="g"
                                    require="true" class="f-input" />
                                <label id="PImageSet" class="hint" runat="server">
                                </label>
                            </div>
                           <div id="Div5" class="field">
                                <label>
                                    引用其他手机图片</label>
                                <input type="text" size="30" style="width: 49%" name="pimget" id="Text1" class="f-input" />
                            </div>
                            <div class="field">
                                <label>
                                    商品图片</label>
                                <asp:FileUpload ID="Image" Style="width: 50%" runat="server" size="30" group="g"
                                    require="true" class="f-input" />
                            ...
                            <div id="img0" class="field">
                                <label>
                                    引用其他图片</label>
                                <input type="text" size="30" style="width: 49%" name="imget" id="imget" class="f-input" />
                            </div>
                            <div class="field">
                                <label>
                                    商品图片1</label>
                                <asp:FileUpload ID="Image1" Style="width: 50%" runat="server" size="30" class="f-input" />
                            ...
                            <div id="img1" class="field">
                                <label>
                                    引用其他图片1</label>
                                <input type="text" size="30" style="width: 49%" name="imget1" id="imget1" class="f-input" />
                            </div>
                            <div class="field">
                                <label>
                                    商品图片2</label>
                                <asp:FileUpload ID="Image2" Style="width: 50%" runat="server" size="30" class="f-input" />
                            ...
                            <div id="img2" class="field">
                                <label>
                                    引用其他图片2</label>
                                <input type="text" size="30" style="width: 49%" name="imget2" id="imget2" class="f-input" />
                            </div>
                            <div class="field">
                                <label>
                                    FLV视频短片</label>
                                <input type="text" size="30" style="width: 49%" name="flv" id="createflv" class="f-input"
                                    runat="server" />
                            ...
                            <div class="field">
                                ...
                                <div style="float: left;">
                                    <textarea cols="45" style="height: 500px;" rows="5" name="detail" id="createdetail"
                                        group="g" require="true" class="f-textarea xheditor {upImgUrl:'../upload.aspx?immediate=1&t=0',urlType:'abs'}"
                                        runat="server"></textarea></div>
                            </div>
                            ...
                            <div id="express_zone_express" style="<%=kuaidi %>">
                                ...
                                <div class="field">
                                    <label>
                                        货到付款接口</label>
                                    <asp:DropDownList class="f-input" ID="ddlCashOnDelivery" Style="float: left; width: 10%"
                                        runat="server">
