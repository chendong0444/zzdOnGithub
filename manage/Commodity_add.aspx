...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        //判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_MallTeam_Add))
        {
            SetError("你不具有新建商城项目的权限！");
        ...
        if (!IsPostBack)
        {
            ...
            initdorp();
            //项目分类
            this.ddlparent.Items.Add(new ListItem("==请选择项目分类==", "0"));
    ...
    private void addContent()
    {
        ...
        team.commentscore = AS.Common.Utils.Helper.GetDecimal(Request["commentscore"], 0);//评论返利的金额
        ...
        if (ddlProduct.Value != "" && ddlProduct.Value != null && ddlProduct.Value != "-----")
        {
            ...
            if (strindexof == -1)
            {
                iproduct = GetProductData("", ddlProduct.Value);
            }
            else
            {
                iproduct = GetProductData("", ddlProduct.Value.Substring(strindexof + 1, ddlProduct.Value.Length - strindexof - 1));
            }
            if (iproduct != null)
            {
                                ...
                                <div class="box-content">
                                    ...
                                    <div class="sect">
                                        ...
                                        <div class="field">
                                            ...
                                            <input type="text" id="ddlProduct" name="ddlProduct" class="f-input" style="width: 360px;" runat="server" value=""
                                                onclick="this.focus(); this.select();" onblur="setValue()" />
                                        </div>
                                        <div class="field" id="dhost">
                                            <label>
                                                首页推荐</label>
                                            <select id="hostt" name="hostt" style="width: 160px;" class="f-input" onchange="stateType()">
                                    ...
                                    <input type="hidden" name="system" value="Y" id="Hidden2" runat="server" />--%>
                                        ...
                                        <div class="field">
                                            <label>
                                                商户</label><div>
                                                    <input type="text" id="shanghu" runat="server" style="float: left; width: 30%;" class="f-input" value=""
                                        ...
                                        <div class="field">
                                            ...
                                            <div style="margin-top: 5px;">
                                                <div id="express" class="daoru">
                                                    <input id="zonediv2" type="radio" name="radio" value="express" checked="checked" />快递
                                                </div>
                                            </div>
                                        </div>
    ...
    <script type="text/javascript">
        function setValue() {
            ...
            if (str != "" & str != "0") {
                $.ajax({
                    type: "POST",
                    url: "manage_ajax_getproduct.aspx?ty=mall",
                    data: { "strproduct": str },
                    async: false,
                    success: function (msg) {
                        var str = msg;
                        ...
                        $("#teamPrice").attr("value", strmsg[2].split('|_|')[0]); //市场价
                        $('select#ddlbrand').attr('value', strmsg[3].split('|_|')[0]); //品牌分类
                        $("#createdetail").val(strmsg[4].split('|_|')[0]); //本单详情
                        if (strmsg[5].split('|_|')[0] == "false") {
                            $("#ddlinventory").attr("disabled", "disabled");
                        }
                        if (strmsg[6].split('|_|')[0] != "" && strmsg[6].split('|_|')[0] != "0" && strmsg[6].split('|_|')[0] != null) {
                            $("#divinven").attr("style", "display:inline-block");
                            $("#p_invent").html(strmsg[6].split('|_|')[0]); //库存数量
                        }
                        else {
                            $("#divinven").attr("style", "display:inline-block");
                            $("#p_invent").html("0"); //库存数量
                        }

                        if (strmsg[7].split('|_|')[0] == 0 || strmsg[8].split('|_|')[0] == "") {
                            $("#shanghu").attr("value", "");
                            $("#hidshanghu").attr("value", "");
                            $("#shHidden").attr("value", "");
                        }
                        else {
                            $("#shanghu").attr("value", strmsg[7].split('*')[0] + ":" + strmsg[8].split('*')[0]); //商户
                            $("#hidshanghu").attr("value", strmsg[7].split('*')[0] + ":" + strmsg[8].split('*')[0]);
                            $("#shHidden").attr("value", strmsg[7].split('*')[0]);
                        }
                        $("#ImageSet").text(strmsg[9].split('|_|')[0]); //商品图片
                        $("#hidImageSet").attr("value", strmsg[9].split('|_|')[0]);
                        $("#tra").html(strmsg[10].split('|_|')[0]); //规格
                        if (strmsg[11].split('|_|')[0] == "True") {
                            $("#teamPrice").attr("disabled", "disabled");
                        }
                        else {
                            $("#teamPrice").attr("disabled", "");
                        }
                        $('select#ddlinventory').attr('value', strmsg[12].split('|_|')[0]); //是否开启库存
                        $("#hidddlinventory").val(strmsg[12].split('|_|')[0]);
                        /*------------根据商户绑定对应的销售人员和分站-------------*/
                        var partnerid = strmsg[7].split('*')[0];
