...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Order_UnPay_ListView))
        {
            SetError("你不具有查看未付款订单列表的权限！");
    ...
    private void InitData()
    {
        ...
        sb1.Append("<th width='5%' >支付方式</th>");
        sb1.Append("<th width='8%' >余付</th>");
        sb1.Append("<th width='10%' >在线支付货到付款</th>");
        sb1.Append("<th width='10%' >递送方式</th>");
        sb1.Append("<th width='10%'>操作</th>");
        sb1.Append("</tr>");
        durl = "Dingdan_WeiFuDingdan.aspx?" + url + "&page=" + AS.Common.Utils.Helper.GetInt(Request.QueryString["page"], 1) + "";
        url = url + "&page={0}";
        url = "Dingdan_WeiFuDingdan.aspx?" + url.Substring(1);
        if (Request["key"] == "3")
        {
            filter.Wheresql1 = " state!='scorepay' and state!='scoreunpay' and (state='unpay' or state='nocod')";
        }
        else
        {
            filter.State = "unpay";
        ...
        if (list_order != null && list_order.Count > 0)
        {
            foreach (IOrder order in list_order)
            {
                ...
                if (user != null)
                {
                    sb2.Append("<a class='remove-record' href='Dingdan_DangqiDingdan.aspx?id=" + order.Id + "' ask='确定本单为现金付款?'>现金</a>｜");
                }
                sb2.Append("<a class='ajaxlink' href='ajax_manage.aspx?action=orderview&orderview=" + order.Id + "'>详情</a></td>");
                sb2.Append("</tr>");
            }
                    ...
                    <div id="content" class="coupons-box clear mainwide">
                        <div class="box clear">
                            <div class="box-content">
                                <div class="head">
                                    <h2>
                                        未付款订单</h2><form id="Form1" runat="server" method="get">
                                <div class="search">
