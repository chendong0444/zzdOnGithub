...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        //判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Flow_Coupon))
        {

            SetError("你不具有查看优惠券消费返利的权限！");
        ...
        if (Request.QueryString["item"] != null)
        {
            ...
            foreach (string ids in item)
            {
                ...
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    ...
                    {
                        SetSuccess("删除选中成功！");
                        AS.Common.Utils.WebUtils.LogWrite("删除财务记录日志", "《删除优惠券消费记录》FlowId:" + id + ",操作员ID：" + AS.Common.Utils.WebUtils.GetLoginAdminID() + "");
                    }
                    else
                    {
                        SetError("删除选中失败！");
            ...
            AS.AdminEvent.ConfigEvent.AddOprationLog(AS.Common.Utils.Helper.GetInt(AS.Common.Utils.CookieUtils.GetCookieValue("admin", key), 0), "删除优惠券消费返利记录", "删除优惠券消费返利记录 ID:" + Request.QueryString["item"].ToString(), DateTime.Now);
            Response.Redirect("CaiWu_Coupon.aspx?page=" + Helper.GetInt(Request.QueryString["url"], 1));
        }
    ...
    private void InitData()
    {
        ...
        sb1.Append("<th width='25%'>优惠券编号</th>");
        sb1.Append("<th width='20%'>Email/用户名</th>");
        sb1.Append("<th width='15%'>动作</th>");
        sb1.Append("<th width='15%'>金额</th>");
        sb1.Append("<th width='20%'>消费时间</th>");
        sb1.Append("</tr>");

        
        url = url + "&page={0}";
        url = "CaiWu_Coupon.aspx?" + url.Substring(1);
        filter.Action = "coupon";
        filter.PageSize = 30;
        ...
        if(iListFlow !=null && iListFlow.Count>0)
        {
            foreach (IFlow flow in iListFlow)
            {
                ...
                sb2.Append("<td >" + flow.Detail_id + "</td>");
 ...
 <script type="text/javascript">
     ...
     function GetDeleteItem(url) {
         ...
         if (str.length > 0) {
             ...
             if (istrue) {
                 window.location = "CaiWu_Coupon.aspx?item=" + $("#items").val() + "&url=" + urls;
             }
