...
<script runat="server">
    ...
    PayFilter payf = new PayFilter();

    protected override void OnLoad(EventArgs e)
    {
        base.OnLoad(e);
        // 判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Flow_ChargeLog_OnLine_ListView))
        {
            SetError("你不具有查看在线充值记录的权限！");
        ...
        if (Request.QueryString["item"] != null)
        {
            ...
            foreach (string ids in item)
            {
                ...
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    ...
                    {
                        SetSuccess("删除选中成功！");
                        AS.Common.Utils.WebUtils.LogWrite("删除财务记录日志", "《删除在线充值记录》FlowId:" + id + ",操作员ID：" + AS.Common.Utils.WebUtils.GetLoginAdminID() + "");
                    }
                    else
                    {
                        SetError("删除选中失败！");
            ...
            AS.AdminEvent.ConfigEvent.AddOprationLog(AS.Common.Utils.Helper.GetInt(AS.Common.Utils.CookieUtils.GetCookieValue("admin", key), 0), "删除在线充值记录", "删除在线充值记录 ID:" + Request.QueryString["item"].ToString(), DateTime.Now);
            Response.Redirect("CaiWu_Charge.aspx?page=" + Helper.GetInt(Request.QueryString["url"], 1));
        }
            ...
            if (!string.IsNullOrEmpty(selTxt))
            {
                ...
                else if (select_type == "3")
                {
                    ...
                    using (IDataSession sesion = AS.GroupOn.App.Store.OpenSession(false))
                    {
                        ...
                        IPay pay = sesion.Pay.Get(payf);
                        if (pay != null)
                            filter.Detail_id = pay.Id;
                    }
                }
    ...
    private void InitData()
    {
        ...
        sb1.Append("<th width='20%'>金额</th>");
        sb1.Append("<th width='20%'>支付方式</th>");
        sb1.Append("<th width='20%'>时间</th>");
        sb1.Append("</tr>");

        url = url + "&page={0}";
        url = "CaiWu_Charge.aspx?" + url.Substring(1);
        filter.Action = "charge";
        filter.PageSize = 30;
        ...
        foreach (IFlow  Flow in iListFlow)
        {
           ...
           pay =  Flow.Pay;
            ...
            sb2.Append("<td >在线充值</td>");
            sb2.Append("<td >" + Flow.Money + "</td>");

            if (pay != null)
                sb2.Append("<td >" + pay.Bank + "</td>");
            else
                sb2.Append("<td ></td>");
                            ...
                            <div class="box-content">
                                ...
                                时&nbsp;间：
                                 ...
                                 <select id="Select1" name="select_type">
                                                ...
                                                <option value="3" <%if(Request.QueryString["select_type"] == "3"){%> selected="selected"<%} %>>支付方式</option>
                                            </select>
 ...
 <script type="text/javascript">
     ...
     function GetDeleteItem(url) {
         ...
         if (str.length > 0) {
             ...
             if (istrue) {
                 window.location = "CaiWu_Charge.aspx?item=" + $("#items").val() + "&url=" + urls;
             }
