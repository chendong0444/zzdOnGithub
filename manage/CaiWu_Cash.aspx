...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        base.OnLoad(e);
        // 判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Flow_Cash_ListView))
        {

            SetError("你不具有查看现金支付记录的权限！");
        ...
        if (Request.QueryString["item"] != null)
        {
            ...
            foreach (string ids in item)
            {
                ...
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    ...
                    {
                        SetSuccess("删除选中成功！");
                        AS.Common.Utils.WebUtils.LogWrite("删除财务记录日志", "《删除现金支付记录》FlowId:" + id + ",操作员ID：" + AS.Common.Utils.WebUtils.GetLoginAdminID() + "");
                    }
                    else
                    {
                        SetError("删除选中失败！");
            ...
            AS.AdminEvent.ConfigEvent.AddOprationLog(AS.Common.Utils.Helper.GetInt(AS.Common.Utils.CookieUtils.GetCookieValue("admin", key), 0), "删除现金支付记录", "删除现金支付记录 ID:" + Request.QueryString["item"].ToString(), DateTime.Now);
            Response.Redirect("CaiWu_Cash.aspx?page=" + Helper.GetInt(Request.QueryString["url"], 1));
        }
    ...
    private void InitData()
    {
        ...
        IUser userAdmin = Store.CreateUserAdmin();
        ...
        sb1.Append("<th width='25%'>订单时间</th>");
        sb1.Append("</tr>");
        url = url + "&page={0}";
        url = "CaiWu_Cash.aspx?" + url.Substring(1);
        filter.Action = "cash";
        filter.PageSize = 30;
        ...
        foreach (IFlow  Flow in iListFlow)
        {
           ...
           userAdmin = Flow.UserAdmin;
            ...
            sb2.Append("<td >现金支付</td>");
            sb2.Append("<td >" + Flow.Money + "</td>");
            if (userAdmin != null)
            {
                sb2.Append("<td >" + userAdmin.Username + "</td>");
            }
            else
            {
                sb2.Append("<td ></td>");
            }
            sb2.Append("<td width='200'>" + Flow.Create_time + "</td>");
            sb2.Append("</tr>");
...
<script type="text/javascript">
    ...
    function GetDeleteItem(url) {
        ...
        if (str.length > 0) {
            ...
            if (istrue) {
                window.location = "CaiWu_Cash.aspx?item=" + $("#items").val() + "&url=" + urls;
            }
