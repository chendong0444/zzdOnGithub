...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        base.OnLoad(e);
         //判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Flow_ChargeLog_BelowLine_ListView))
        {

            SetError("你不具有查看线下充值记录的权限！");
        ...
        if (Request.QueryString["item"] != null)
        {
            ...
            foreach (string ids in item)
            {
                ...
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        AS.Common.Utils.WebUtils.LogWrite("删除财务记录日志", "《删除线下充值记录》FlowId:" + id + ",操作员ID：" + AS.Common.Utils.WebUtils.GetLoginAdminID() + "");
                    }
                    else
                    {
                        SetError("删除选中失败！");
            ...
            AS.AdminEvent.ConfigEvent.AddOprationLog(AS.Common.Utils.Helper.GetInt(AS.Common.Utils.CookieUtils.GetCookieValue("admin", key), 0), "删除线下充值记录", "删除线下充值记录 ID:" + Request.QueryString["item"].ToString(), DateTime.Now);
            Response.Redirect("CaiWu_Store.aspx?page=" + Helper.GetInt(Request.QueryString["url"], 1));
        }
        ...
        if (!string.IsNullOrEmpty(startTime) && !string.IsNullOrEmpty(endTime))
        {
            ...
            url = url + "&startTime=" + Request.QueryString["startTime"] + "&endTime=" + Request.QueryString["endTime"];
    ...
    private void InitData()
    {
        ...
        sb1.Append("<th width='20%'>操作员</th>");
        sb1.Append("<th width='20%'>线下充值时间</th>");
        sb1.Append("</tr>");
        
        url = url + "&page={0}";
        url = "CaiWu_Store.aspx?" + url.Substring(1);
        filter.Action = "store";
        filter.PageSize = 30;
                                ...
                                时&nbsp;间：
                                            <input type="text" readonly="readonly" <%if(!string.IsNullOrEmpty(Request.QueryString["startTime"])){ %> value='<%=Request.QueryString["startTime"] %>'<%} %> name="startTime"
                                                onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" class="date" style="width: 110px"  />到&nbsp;&nbsp;&nbsp;<input
                                                    type="text" readonly="readonly" name="endTime"  <%if(!string.IsNullOrEmpty(Request.QueryString["startTime"])){ %> value='<%=Request.QueryString["endTime"] %>'<%} %>
                                            ...
                                            &nbsp;<input type="text" name="memail" class="h-input" <%if(!string.IsNullOrEmpty(Request.QueryString["memail"])){ %>
                                                value='<%=Request.QueryString["memail"] %>' <% }%> />&nbsp;<input type="submit" value="筛选" class="formbutton"
...
<script type="text/javascript">
    ...
    function GetDeleteItem(url) {
        ...
        if (str.length > 0) {
            ...
            if (istrue) {
                window.location = "CaiWu_Store.aspx?item=" + $("#items").val() + "&url=" + urls;
            }
