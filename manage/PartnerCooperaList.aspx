...
<script runat="server">
    ...
    {  
        base.OnLoad(e);
        //判断管理员是否有此操作
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Business_ListView))
        {
            SetError("你不具有查看商户合作列表的权限！");
        ...
        url = url + "&page={0}";
        url = "PartnerCooperaList.aspx?" + url.Substring(1);
        filter.PageSize = 30;
        ...
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
            ...
            {
                if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Business_Delete))
                {
                    SetError("你不具有删除商户合作信息的权限！");
                    Response.Redirect("PartnerCooperaList.aspx");
                    Response.End();
                    return;
                }
                int i = 0;
        ...
        {
            if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_Business_Handle))
            {
                SetError("你不具有处理商户合作信息的权限！");
                Response.Redirect("PartnerCooperaList.aspx");
                Response.End();
                return;
...
<script type="text/javascript">
    ...
    function GetDeleteItem() {
        ...
        if (str.length > 0) {
            ...
            if (istrue) {
                window.location = "PartnerCooperaList.aspx?item=" + $("#items").val();
            }
                                    ...
                                    <table id="orders-list" cellspacing="0" cellpadding="0" border="0" class="coupons-table">
                                        ...
                                        <%int i = 0;
                                            if (iListFeedback != null && iListFeedback.Count > 0)
                                          {
                                            ...
                                            %>
                                            <td>
                                                <a ask="确认删除吗？" class="remove-record" href="PartnerCooperaList.aspx?del=<%=feedbackInfo.Id %>">
                                                    删除</a> | <a href="PartnerCooperaList.aspx?id=<%=feedbackInfo.Id %>">处理</a>
                                            </td>
                                            <%
                                                }
                                              else
