...
<script runat="server">
    ...
    protected ICategory category = Store.CreateCategory();
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        type = Helper.GetString(Request["action"], String.Empty);
        string update = Request["btnOK"];
        int partner_id = int.Parse(CookieUtils.GetCookieValue("partner", key));
        orfilter.Partner_id = partner_id;
        orfilter.Id = id;
        int i = 0;
        using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
            i = session.Orders.Count(orfilter);
        }
        if (i > 0)
        {
            if (!Page.IsPostBack)
        ...
        <table width="96%" align="center" class="coupons-table-xq">
            ...
            <tr>
                ...
                <td class="style1">
                    ...
                    <%-- <input id="btnOK" name="btnOK" type="submit" runat="server" value="发货" class="validator"
                        group="a" />--%>
                    <input id="Submit1" type="submit" runat="server" class="formbutton" value="发货" />
                    &nbsp&nbsp&nbsp&nbsp;
                    <%if (order.Express_id != 0)
                      { %>
                    <input type="button" class="formbutton" onclick="print()" value="打印快递" />
                    <%} %>
                </td>
            </tr>
            <tr>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        <%if(kuaino!=""){ %>
    X.get("<%=PageValue.WebRoot %>manage/ajax_wuliu.aspx?id=<%=ename %>&&kuai=<%=kuaino %>&&type=1");
    <% }%>
