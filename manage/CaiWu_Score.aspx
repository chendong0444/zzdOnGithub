...
<script runat="server">
        ...
        if (Request.QueryString["item"] != null)
        {
            ...
            foreach (string ids in item)
            {
                ...
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    ...
                    int i = 0;
                    i = session.Scorelog.Delete(id);
                    if (i > 0)
                    {
                        SetSuccess("删除选中成功！");
                        AS.Common.Utils.WebUtils.LogWrite("删除财务记录日志", "《删除积分记录》ScoreId:" + id + ",操作员ID：" + AS.Common.Utils.WebUtils.GetLoginAdminID() + "");
                    }
                    else
                    {
                        SetError("删除选中失败！");
            ...
            AS.AdminEvent.ConfigEvent.AddOprationLog(AS.Common.Utils.Helper.GetInt(AS.Common.Utils.CookieUtils.GetCookieValue("admin", key), 0), "删除积分记录", "删除积分记录 ID:" + Request.QueryString["item"].ToString(), DateTime.Now);
            Response.Redirect("CaiWu_Score.aspx?page=" + Helper.GetInt(Request.QueryString["url"], 1));
        }
    ...
    private void InitData()
    {
        ...
        sb1.Append("<th width='15%'>订单ID</th>");
        sb1.Append("<th width='15%'>积分值</th>");
        sb1.Append("<th width='15%'>动作</th>");
        sb1.Append("<th width='25%'>生成时间</th>");
        sb1.Append("</tr>");

       
        url = url + "&page={0}";
        url = "CaiWu_Score.aspx?" + url.Substring(1);
        filter.PageSize = 30;
        ...
        int i = 0;
        foreach (IScorelog Scorelog in iListScorelog)
        {
            user = Scorelog.User;
            if (i % 2 != 0)
            {
            ...
            i++;
            sb2.Append("<td><input type='checkbox' id='check' name='check' value=" + Scorelog.id + " /></td>");
            if (user != null)
            {
            ...
            sb2.Append("<td >" + Scorelog.key +"</td>");
            sb2.Append("<td >" + Scorelog.score + "</td>");
            sb2.Append("<td >" + Scorelog.action + "</td>");
            sb2.Append("<td >" + Scorelog.create_time + "</td>");
            sb2.Append("</tr>");
        }
                                ...
                                时&nbsp;间：
                                            <input type="text" readonly="readonly" <%if(!string.IsNullOrEmpty(Request.QueryString["startTime"])){ %> value='<%=Request.QueryString["startTime"] %>'<%} %> name="startTime"
                                               ...
                                               <select id="Select1" name="select_type">
                                                ...
                                                <option value="3" <%if(Request.QueryString["select_type"] == "3"){%> selected="selected"<%} %>>订单编号</option>
                                            </select>
 ...
 <script type="text/javascript">
     ...
     function GetDeleteItem(url) {
         ...
         if (str.length > 0) {
             ...
             if (istrue) {
                 window.location = "CaiWu_Score.aspx?item=" + $("#items").val() + "&url=" + urls;
             }
