...
<script runat="server">
    ...
    {
        base.OnLoad(e);
        if (AdminPage.IsAdmin && AdminPage.CheckUserOptionAuth(PageValue.CurrentAdmin, ActionEnum.Option_TJ_Team))
        {
            SetError("你不具有查看项目统计的权限！");
    ...
    private void initPage()
    {
        ...
        if (ViewState["table"] == null)
        {
            ...
            strSql = strSql + ",teamid as [项目ID],product as [项目名称],sum(num) as [购买数量],sum(num*Team_price) as [项目收款] ";
            strGroup = strGroup + ",teamid,product,Team_price";
            ...
            if (drSeviceWhere.SelectedIndex > 0)
                strSql = strSql + " and service='" + drSeviceWhere.SelectedValue + "' ";
            ...
            strSql = strSql + " group by " + strGroup.Substring(1);
            ...
            ViewState["table"] = AS.Common.Utils.Helper.ConvertDataTable(hashtable);
            ...
            msg.Text = "购买数量：" + prices[1] + "; 项目收款: " + prices[2];
        }
        else
        {
            table = (System.Data.DataTable)ViewState["table"];
        }
    ...
    protected void btnDown_Click(object sender, EventArgs e)
    {
        ...
        if (ddlparent.SelectedIndex > 0)
        {
            strSql = strSql + "and (cataid in(" + ddlparent.SelectedValue + "))";
        }
        strSql = strSql + " group by " + strGroup.Substring(1);
        using (AS.GroupOn.DataAccess.IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
        {
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (dr.Row.Table.Columns["项目"] != null)
            {
                ...
                using (IDataSession session = AS.GroupOn.App.Store.OpenSession(false))
                {
                    team = session.Teams.GetByID(int.Parse(dr["项目"].ToString()));
                }
                if (team != null)
                {
                        ...
                        <div class="box clear">
                            <div class="box-content">
                                <div class="head">
                                   <h2>项目统计</h2>
                                    <div class="lie_fl">
                                        筛选条件：项目编号：<input id="teamid" type="text" runat="server"  class="h-input" />
                                        &nbsp;付款日期：<input id="beginTime" type="text" style="margin-right:0px;" runat="server" onclick="WdatePicker();" class="h-input"/>--<input
