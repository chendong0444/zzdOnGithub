...
<script runat="server">
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        url = url + "&page={0}";
        url = "Dingdan_WeiXuanZe.aspx?" + url.Substring(1);
                                        ...
                                        <%} %> />--<input type="text" class="h-input" datatype="date" name="endtime" <%if(!string.IsNullOrEmpty(Request.QueryString["endtime"])){ %>value="<%=Request.QueryString["endtime"] %>"
                                            <%} %> />&nbsp;&nbsp; 完成订单时间：<input type="text" class="h-input" datatype="date" name="fromfinishtime" style="margin-right:0px;"
                                                ...
                                                <%} %> />--<input type="text" class="h-input" datatype="date" name="endfinishtime"
                                                    ...
                                                    内容：<input type="text" style="width: 120px;" name="type_name" class="h-input"
                                    ...
                                    <table id="orders-list" cellspacing="0" cellpadding="0" border="0" class="coupons-table">
                                        ...
                                        <td> <input type="checkbox" name="check" id="check" value='<%=order.Id %>' /></td>
                                            ...
                                            <td class='xiangmu'>
                                                ...
                                                %>
                                                项目ID:<%=teams.Teamid%>
                                                (<a class="deal-title" href='<%=getTeamPageUrl(teams.Team.Id)%>' target="_blank"><% =StringUtils.SubString(teams.Team.Title, 70) + "..."%></a>)
                                                <%=StringUtils.SubString(AS.Common.Utils.WebUtils.Getbulletin(teams.result), 0, "<br>")%>
                                                <%
                                                    }
}
else
{ %>
                                                项目ID:<%=order.Team_id%>
                                                (<a class="deal-title" href='<%=getTeamPageUrl(order.Team.Id)%>' target="_blank">
                                                    <%=StringUtils.SubString(order.Team.Title, 70) + "..."%>
                                                </a>)
                                                <% 
}
